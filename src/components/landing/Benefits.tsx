...
export function Benefits() {
  return (
    <section id="beneficios" className="bg-[var(--surface-card)]">
      <div className="mx-auto max-w-[1120px] px-6 py-22">
        ...
        <div className="grid grid-cols-1 gap-6 md:grid-cols-3">
          {ITEMS.map(({ icon: Icon, title, body }) => (
            <div key={title} className="rounded-xl border border-[var(--border-default)] bg-[var(--surface-card)] p-[30px]">
              ...
              <p className="mt-2.5 text-sm leading-relaxed text-[var(--text-muted)]">{body}</p>
            </div>
          ))}
        </div>
      </div>
