    ...
    <section className="bg-white">
      <div className="mx-auto grid max-w-[1120px] grid-cols-1 items-center gap-16 px-6 pb-20 pt-22 lg:grid-cols-[1.05fr_0.95fr]">
        <div>
          ...
          <h1 className="text-balance text-5xl font-semibold leading-[1.08] tracking-tight text-gray-900">
            A gestão da sua clínica odontológica, sem complicação.
          </h1>
          <p className="mt-6 max-w-[30em] text-lg leading-relaxed text-gray-500">
        ...
        <div className="relative">
          <div className="overflow-hidden rounded-xl border border-gray-200 bg-white shadow-[0_24px_60px_rgba(16,24,40,0.10)]">
            <div className="flex items-center justify-between border-b border-gray-100 px-5 py-4">
              <div className="flex items-center gap-2.5">
                <Calendar className="h-[18px] w-[18px] text-cliniq-600" />
                <span className="text-sm font-medium text-gray-900">Agenda · Hoje</span>
              </div>
              <span className="font-mono text-xs text-gray-400">17/06</span>
            </div>
            <div className="py-2">
              {APPTS.map((a, i) => (
                <div
                  key={a.name}
                  className={`flex items-center justify-between px-5 py-3 ${i < APPTS.length - 1 ? "border-b border-gray-100" : ""}`}
                >
                  <div>
                    <div className="text-sm font-medium text-gray-900">{a.name}</div>
                    <div className="font-mono text-xs text-gray-500">{a.info}</div>
                  </div>
