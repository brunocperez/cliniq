...
export default async function DashboardPage() {
  ...
  const [
    ...
    supabase
      .from('appointments')
      .select('*, patients(name), services(price)')
      .eq('tenant_id', tenantId)
  ...
  // Calcula receita hoje a partir dos dados já buscados
  const receitaHoje = consultasHoje
  ...
  const temConsultasHoje = (consultasHoje?.length ?? 0) > 0
    ...
    <div>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            ...
            <div>
              <p style={{ margin: 0, fontSize: 'var(--text-sm)', fontWeight: 'var(--weight-medium)', color: 'var(--brand)' }}>
                Você tem {consultasHoje?.length} consulta{(consultasHoje?.length ?? 0) > 1 ? 's' : ''} hoje
              </p>
              {consultasHoje && consultasHoje.length > 0 && (
                <p style={{ margin: '2px 0 0', fontSize: 'var(--text-xs)', color: 'var(--text-muted)' }}>
                  Próxima: {(consultasHoje[0].patients as { name: string } | null)?.name ?? 'Paciente'} às{' '}
                  {new Date(consultasHoje[0].scheduled_at).toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit' })}
                </p>
              )}
            </div>
          </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 16, marginBottom: 24 }}>
        ...
        <div style={{ borderRadius: 'var(--radius-lg)', padding: 20, color: 'white', background: 'var(--cliniq-500)' }}>
          ...
          <p style={{ margin: '4px 0 0', fontSize: 'var(--text-2xl)', fontWeight: 'var(--weight-medium)' }}>{consultasHoje?.length ?? 0}</p>
        </div>
